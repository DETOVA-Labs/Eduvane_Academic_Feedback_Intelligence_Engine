...
export const Dashboard: React.FC<DashboardProps> = ({ onAction, onCommand, submissions, profile }) => {
  ...
  const handleCommandSubmit = async (e: React.FormEvent) => {
    ...
    setIsRouting(true);
    try {
      // Logic: Gemini-3-flash-preview processes input for Intent & Metadata
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      {/* Eduvane Intel-Link: Command Bar */}
      <div className="bg-[#1E3A5F] p-10 rounded-[2.5rem] shadow-2xl text-white relative overflow-hidden border-b-8 border-[#1FA2A6]">
        <div className="relative z-10 space-y-6">
          ...
          <div>
            <h2 className="text-3xl font-black tracking-tight">Welcome, {profile?.email.split('@')[0]}</h2>
            <p className="text-slate-400 text-sm max-w-lg mt-1 font-medium italic">Command the intelligence core. Practice, analyze, or review.</p>
          </div>
          ...
          <form onSubmit={handleCommandSubmit} className="relative group">
            <input 
              ...
              placeholder="e.g., 'Start practice for Calculus' or 'Evaluate my essay'..."
              className="w-full bg-white text-[#1E3A5F] border-4 border-transparent rounded-2xl py-5 px-8 pr-16 outline-none focus:border-[#1FA2A6] transition-all placeholder:text-slate-400 font-bold shadow-2xl text-lg"
              disabled={isRouting}
            />
            <button 
              type="submit"
              disabled={isRouting}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-4 bg-[#1E3A5F] rounded-xl text-[#1FA2A6] shadow-lg hover:bg-slate-800 transition-all disabled:opacity-50"
            >
          ...
          <div className="flex gap-6 text-[10px] font-black uppercase tracking-widest text-slate-500">
            <span className="flex items-center gap-1.5"><Sparkles size={12} className="text-[#1FA2A6]" /> Intent Routing Active</span>
            <span className="flex items-center gap-1.5"><Search size={12} className="text-[#1FA2A6]" /> Subject Inference Ready</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <section className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-200 hover:border-[#1FA2A6]/30 transition-all group">
          ...
          <div className="flex items-end justify-between">
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2">Overall Growth Score</p>
            </div>
        ...
        <section className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-200 hover:border-[#1FA2A6]/30 transition-all">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h3 className="font-black text-[#1E3A5F] text-xs uppercase tracking-widest">Timeline Signals</h3>
          </div>
          <div className="space-y-4">
            {submissions.slice(0, 3).map(s => (
              <div key={s.id} className="flex justify-between items-center group cursor-pointer hover:bg-slate-50 p-3 rounded-xl transition-all border border-transparent hover:border-slate-100">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 uppercase font-mono tracking-tighter">{new Date(s.timestamp).toLocaleDateString()} • Inferred</p>
                </div>
            ...
            {submissions.length === 0 && (
              <div className="text-center py-6">
                <p className="text-xs text-slate-400 italic mb-4">No learning signals indexed yet.</p>
                <button 
